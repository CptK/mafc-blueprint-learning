...
from mafc.tools.web_search.common import SearchResults, Query, WebSource
...
class GoogleSearchResults(SearchResults):
    answer: str | None = None  # sometimes, Google provides a comprehensive answer
    ...
    def __repr__(self):
        return (
            f"GoogleSearchResults(n_sources={len(self.sources)}, "
...
class SerperAPI:
    ...
    def __init__(self, gl: str = "us", hl: str = "en", tbs: str | None = None, **kwargs):
        super().__init__(**kwargs)
        self.serper_api_key = self._resolve_api_key()
        self.gl = gl
        self.hl = hl
        self.tbs = tbs

    def _resolve_api_key(self) -> str | None:
        """Resolves the Serper API key from environment variables."""
    ...
    def search(self, query: Query) -> GoogleSearchResults | None:
        ...
        return GoogleSearchResults(
            sources=web_sources, answer=answer, knowledge_graph=knowledge_graph, query=query
        )
    ...
    def _parse_results(
        self, response: dict[Any, Any], query: Query
    ) -> tuple[str | None, str | None, list[WebSource]]:
    ...
    def _parse_sources(self, response: dict, query: Query) -> list[WebSource]:
        ...
        filtered_results = filter_unique_results_by_domain(result_bucket)
        for result in filtered_results:
            # somehow the num param does not restrict requests.post image search results -> break here
            if query.limit and len(sources) >= query.limit:
                break
...
def _parse_answer_box(response: dict) -> str | None:
