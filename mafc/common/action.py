...
def get_action_documentation(action_cls: type[Action]) -> str:
    ...
    desciton_doc = inspect.getdoc(action_cls)
    description = desciton_doc.strip() if desciton_doc else "No description provided."
    usage_doc = inspect.getdoc(action_cls.__init__)
    usage = usage_doc.strip(" \t\n") if usage_doc else "No usage documentation provided."
